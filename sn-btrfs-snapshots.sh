...
count_save=30
sudo btrfs subv list / | grep -E "/home.|/root." | head -n+$(($count_all -$count_save ))| cut -d ' ' -f 9 | xargs -i echo /{} | xargs -i sudo btrfs subvolume delete {}
Exit

Скрипт создает снимки сабвольюмов /@ и /@home в директорию /.snapshots и удаляет все снимки кроме 30 последних. Число хранимых снимков можно изменить отредактировав переменную count_save.
